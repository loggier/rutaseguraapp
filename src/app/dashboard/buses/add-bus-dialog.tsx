...
type AddBusDialogProps = {
  isOpen: boolean;
  onClose: () => void;
  onBusAdded: (newBus: Autobus) => void;
  user: User;
};

export function AddBusDialog({ isOpen, onClose, onBusAdded, user }: AddBusDialogProps) {
  ...
  useEffect(() => {
      ...
      async function fetchForColegioUser() {
          if(user.rol !== 'colegio') return;
           const supabase = createClient();
  ...
  }, [isOpen, user, watchedColegioId, form])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Error al crear el autobús.');
      }
      
      toast({
        title: 'Éxito',
        description: 'El autobús ha sido creado correctamente.',
      });
      onBusAdded(data.bus);
      onClose();
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Agregar Nuevo Autobús</DialogTitle>
            <DialogDescription>
              Completa los campos para registrar un nuevo autobús.
            </DialogDescription>
          </DialogHeader>
